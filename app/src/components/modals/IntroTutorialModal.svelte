                ...
                <TutorialStep>
                    <div class="text-center">
                        ...
                        <TutorialElement customClass="mt-10 text-2xl" index="1">
                            Futhermore, <strong>persistent reminders</strong> will not expire the first time and they keep sending you notifications until you manually dismiss them: so don't worry if you
                            are too busy to do it now, you won't forget that important call!
                        </TutorialElement>
                    </div>
                </TutorialStep>
            {:else if step === 6}
                <TutorialStep>
                    <div class="text-center">
                        ...
                        <TutorialElement customClass="mt-10 text-xl" index="2">
                            <strong>{altLabel} + R</strong> Open the command box to set a reminder <br />
                            <strong>{altLabel} + Shift + P</strong> Create a new pinned message <br /><br />
                            <em><strong>For Spotify:</strong></em><br />
                            <strong>{altLabel} + M</strong> Search a playlist on Spotify
                        </TutorialElement>
                    </div>
                </TutorialStep>
            {:else if step === 7}
        ...
        <footer class="flex flex-col justify-center justify-self-end bg-primary z-10">
            <div class="flex justify-center mb-3">
                {#each Array(9) as _, index}
                    <i class="mx-1 fas fa-circle {step === index ? 'text-primary' : 'text-secondary'} text-xxs cursor-pointer" on:click={() => (step = index)} />
                {/each}
            </div>
            <div class="flex flex-row justify-center justify-self-end">
                ...
                <Action label="back" customClass="border-2 border-transparent bg-transparent mx-2 {step > 0 ? '' : 'cursor-not-allowed'}" custom on:click={() => (step === 0 ? 0 : step--)} />
